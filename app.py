...
def synthetic_data():
    ...
    if st.session_state.data is not None:
        ...
        if st.session_state.synthetic_data is not None:
            ...
            if st.button("Validation Report"):
                real_data = data.copy()
                metadata = Metadata.detect_from_dataframe(data=real_data, table_name='water_potability_augmented')
                
                # Capture console output during diagnostic/quality evaluation.
                buffer = io.StringIO()
                ...
                st.subheader("Validation Report Output")
                # Wrap the captured output in triple backticks for code formatting.
                formatted_output = f"\n{output_text}\n"
                st.markdown(formatted_output)
                
                st.subheader("Column Plots Comparison")
                columns_to_plot = [
                ...
                for column_name in columns_to_plot:
                    if column_name in real_data.columns:
                        fig = get_column_plot(
                            real_data=real_data,
